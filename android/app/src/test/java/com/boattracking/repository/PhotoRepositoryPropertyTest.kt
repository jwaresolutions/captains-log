...
class PhotoRepositoryPropertyTest : StringSpec({
    ...
    "Property 51: Local Photo Upload Preference - Connection types support local preference".config(
        ...
        checkAll<Int>(Arb.int(0..2)) { typeIndex ->
            ...
            val connectionTypes = com.boattracking.connection.ConnectionManager.ConnectionType.values()
            
            // Property: Should have LOCAL, REMOTE, and NONE connection types
            connectionTypes.size shouldBe 3
            connectionTypes.contains(com.boattracking.connection.ConnectionManager.ConnectionType.LOCAL) shouldBe true
            connectionTypes.contains(com.boattracking.connection.ConnectionManager.ConnectionType.REMOTE) shouldBe true
            connectionTypes.contains(com.boattracking.connection.ConnectionManager.ConnectionType.NONE) shouldBe true
            
            // Property: LOCAL should be a valid connection type for preference
            val localType = com.boattracking.connection.ConnectionManager.ConnectionType.LOCAL
            localType.name shouldBe "LOCAL"
            
            // Property: REMOTE should be available as fallback
            val remoteType = com.boattracking.connection.ConnectionManager.ConnectionType.REMOTE
